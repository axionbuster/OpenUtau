...
final class Host: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        AVAudioUnit.instantiate(with: desc, options: [.loadInProcess]) { node, error in
            DispatchQueue.main.async {
                ...
                else { DispatchQueue.main.asyncAfter(deadline: .now() + 2) { self.render() } }
            }
        }
    }
    func showSetup() {
    ...
    func render() {
        guard let j = job, let node = node else { return }
        ...
        do {
            ...
            let buffer = AVAudioPCMBuffer(pcmFormat: fmt, frameCapacity: 1024)!
            var previous: [Float]?
            for attempt in 0..<8 {
                print("Preparing VoiSona (\(attempt + 1)/8)")
                try warm(attempt == 0 ? 3 : 2, buffer)
