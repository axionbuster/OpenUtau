...
OpenUtau applies dynamics to the completed audio. The renderer uses one phonemized lyric per note. UTAU phonemizers that emit multiple sample aliases per note are rejected; use DEFAULT or KO to JA. Unconverted Hangul is rejected before synthesis, rather than accepting the Japanese engine's near-silent output. Native voice controls are described below. The renderer does not advertise sample-level phoneme timing or rendered-pitch extraction. For English words, `+` advances to the next syllable while one remains; `+~` and `+*` sustain the current syllable. Once all syllables have been assigned, further extension notes sustain the last syllable. The adapter uses OpenUtau's bundled English pronunciation dictionary/G2P to assign phonemes at those explicit note boundaries, then supplies VoiSona's native `Phoneme` fields. Unmarked words and monosyllabic words retain native pronunciation. These dictionary-derived pronunciations may differ from VoiSona's own choices. For Japanese/KO-to-JA, extension chains continue to sustain the converted lyric. Exact OpenUtau pitch curves are preserved across extensions.

English pronunciation hints use OpenUtau's bracket notation, with VoiSona English phoneme symbols: for example, `hmm [mm]` requests a syllabic m (closed-mouth hum), and `hmm [m]` requests an ordinary consonant m. Hints are translated into native phoneme fields, not sent as lyric text. A hint overrides the automatic pronunciation and follows the same extension rules. The [VoiSona phoneme table](https://manual.voisona.com/en/song/pc/2b6e9bc7efb18077b49cce445844cd95) lists the supported symbols. No project marker conversion is needed. An extension without a preceding lyric or across a rest is rejected. Overlapping notes and individual native render jobs longer than ten minutes are rejected with an error.
...
This reduces the work needed before useful audio reaches the playhead. During VoiSona playback, OpenUtau waits for the selected/focused audible part's overlapping chunk when one exists; otherwise it waits for any audible overlapping chunk. Muted or solo-suppressed tracks do not release that gate, and a true timeline gap advances immediately. After the first useful chunk arrives, playback deliberately keeps time as an incomplete audition mix: Pending tracks contribute silence and join when ready. A session-only 20 ms ramp begins at the current transport position when a late chunk becomes audible, without modifying its durable cache PCM. Export still waits for every required slot and never uses incomplete-mix semantics.

Playback prioritizes the focused playhead chunk, and unfocused background preparation warms chunks in timeline order across tracks. Playback and background preparation admit up to two independent native jobs at a time, in priority order, and publish each completed chunk immediately. Mixdown and stem export admit up to four jobs across tracks. Both limits are reduced to half the logical processor count (at least one); a process-wide four-job ceiling bounds overlapping passes. Exports take the next available native slot before queued playback/background work. Already running work completes so its cache remains reusable. Playback/background hosts run with niceness +10; export helpers retain normal process priority. Identical cache entries are locked during rendering and writes, before requesting a native slot. Cache hits and duplicate-cache waiters consume no native slots. Cancellation or a failed job cancels and reaps the remaining jobs in that pass. A long sustained syllable or a phrase made entirely of separated notes can exceed the target. Repeated context can increase total render work, and independent synthesis can change pronunciation/timbre at chunk boundaries despite the context and crossfade; the audio is not promised to equal a single whole-phrase render.
...
Live playback uses a bounded pool of at most two persistent helper processes, each with one VoiSona Audio Unit and one sequential job at a time. A helper is retired after two idle minutes. Every request has a unique generation recorded in both the atomic result and the server completion message, so canceled or stale output cannot satisfy a newer request. Cancellation, process death, a protocol mismatch, invalid metadata, or a native state/render failure kills and replaces that host; an infrastructure failure retries once through the established isolated one-shot helper. Mixdown and stem export retain fresh one-shot helpers and the same strict chunk/cache format as playback.

Each helper starts the audio graph before loading musical state and pumps the native event loop with stopped transport while VoiSona prepares. Warm hosts shorten their first two readiness periods, but the plugin's `PluginState` parameter is not treated as a readiness signal. The same consecutive-capture acceptance rule applies to persistent and one-shot rendering; preview audio is not written to the durable cache before that transaction succeeds.
...
### English boundary verification (2026-09-16)

A controlled AU state-readback probe on VoiSona 1.18.0.5 tested `early, +~, +` and `hmm [mm]`. Supplying `Phoneme` alone did not retain the edits: after synthesis, both early notes contained `axr*~|l,iy`, and the hum contained `hh,ax,m`. Supplying matching `Phoneme`, `DefaultPhoneme`, and `PastAnalyzedPhoneme`, together with `AnalyzedNoteLanguage=2`, retained `axr`, `l,iy`, and `mm` in the prepared native state. The first syllable occupied 500–1500 ms, the second 1500–2250 ms, and the hum 2750–4250 ms in the fixture. This verifies native acceptance of the requested phonemes and score boundaries; it is not an independent pronunciation/timbre audition.

The delivered `0.1.570-tet31.17` bundle passed deep-strict signature verification with matching build/installed main and Core DLL hashes. The focused regression suite passed 58 tests (10 opt-in native tests skipped); the boundary suite with native synthesis enabled passed all eight tests. A separate installed-assembly mixdown of the four-track, eight-part `ant5-chis-a` project completed in 219.48 seconds, followed by a successful finite/nonzero playback check using the shared cache. The exported M4A was stereo 44.1 kHz AAC, 125.550295 seconds, with decoded peak -2.713 dBFS and no NaN/Inf samples. The original audio was hash-preserved; only the four `[m]` humming hints changed to `[mm]` in the backed-up project. This was a headless export using the installed Core assembly and signed helper, not a GUI export or independent audition.
