...
namespace OpenUtau.App.Controls {
    class NotesCanvas : Control {
        ...
        private void RenderPitchBend(UNote note, NotesViewModel viewModel, DrawingContext context) {
            ...
            Point p0 = viewModel.TickToneToCenterPoint(p0Tick, p0Tone);
            Point p_1 = p0;
            ...
            for (int i = 1; i < pts.Count; i++) {
                ...
                Point p1 = viewModel.TickToneToCenterPoint(p1Tick, p1Tone);
                ...
                if (pts.Count > 2 && pts[i - 1].shape == PitchPointShape.sp) {
                    ...
                    if (i == 1) {
                        if (note.pitch.data[0].X > 0) {
                            p_1 = viewModel.TickToneToCenterPoint(note.position, p0Tone);
                        }
                    }
                    if (i < pts.Count - 1) {
                        ...
                        p2 = viewModel.TickToneToCenterPoint(p2Tick, p2Tone);
                    } else if (pts[i].X < note.DurationMs) {
                        p2 = viewModel.TickToneToCenterPoint(note.End, note.AdjustedTone);
                    }
        ...
        private void RenderVibrato(UNote note, NotesViewModel viewModel, DrawingContext context) {
            ...
            var points = new Points();
            points.Add(viewModel.TickToneToCenterPoint(point.X, point.Y));
            while (nPos < 1) {
            ...
            lock (Part!) {
                foreach (var phrase in Part!.renderPhrases) {
                    ...
                    for (int i = startIdx; i < endIdx; ++i) {
                        ...
                        points.Add(viewModel.TickToneToCenterPoint(t, p / 100));
                    }
                    var geometry = new PolylineGeometry(points, false);
                    context.DrawGeometry(null, pen, geometry);
                }
