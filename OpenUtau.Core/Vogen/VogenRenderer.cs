...
namespace OpenUtau.Core.Vogen {
    public class VogenRenderer : IRenderer {
        ...
        public Task<RenderResult> Render(RenderPhrase phrase, Progress progress, int trackNo, CancellationTokenSource cancellation, bool isPreRender = false, RenderPhraseEvents? renderEvents = null) {
            var task = Task.Run(() => {
                lock (lockObj) {
                    ...
                    var wavPath = Path.Join(PathManager.Inst.CachePath, $"vog-{phrase.hash:x16}.flac");
        ...
        float[] InvokeVogen(RenderPhrase phrase) {
            ...
            var f0 = SampleCurve(phrase, phrase.pitches, 0, totalFrames, headFrames, tailFrames, x => phrase.ToneToFrequency(x * 0.01));
            ...
            if (phrase.toneShift != null) {
                for (int i = 0; i < f0.Length - headFrames - tailFrames; i++) {
                    ...
                    if (index < phrase.pitches.Length) {
                        f0Shifted[i + headFrames] = (float)phrase.ToneToFrequency((phrase.pitches[index] + phrase.toneShift[index]) * 0.01);
                    }
                }
            }
        ...
        public RenderPitchResult LoadRenderedPitch(RenderPhrase phrase) {
            ...
            var result = new RenderPitchResult() {
                tones = np.Load<float[]>(f0Path).Select(f => (float)phrase.FrequencyToTone(f)).ToArray(),
            };
